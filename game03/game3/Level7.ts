module Game3 {
    ...
    export class Level7 extends Phaser.State {

        static SPAWN_ROW: number = 8;
        static SPAWN_COLUMN: number = 3;
        static MAX_LIVES: number = 2;
        static ENEMIES_SPEED: number = 600;
        static PLAYER_BULLET_SPEED: number = 280;
        static PLAYER_BULLET_FREQUENCY: number = 320;
        static ENEMY_BULLET_SPEED: number = 320;
        static ENEMY_BULLET_FREQUENCY: number = 800;
        ...
        createEnemies() {
            for (var x = 0; x < Level7.SPAWN_ROW; x++) {
                for (var y = 0; y < Level7.SPAWN_COLUMN; y++) {
                    var enemy: Phaser.Sprite = this.enemies.create((x * (36 * 2)), (y * (44 * 1.5)) + 30, 'enemies');
            ...
            var tween: Phaser.Tween = this.add.tween(this.enemies).to({ x: 200 }, Level7.ENEMIES_SPEED, Phaser.Easing.Linear.None, true, 0, true, true);
            tween.onComplete.add(this.moveEnemiesDown, this);
        }

        moveEnemiesDown() {
            var tween: Phaser.Tween = this.add.tween(this.enemies).to({ y: this.enemies.y + 20 }, Level7.ENEMIES_SPEED / 4, Phaser.Easing.Linear.None, true, 0);

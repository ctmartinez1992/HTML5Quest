module Game3 {
    ...
    export class Level4 extends Phaser.State {
        ...
        static SPAWN_ROW: number = 12;
        static SPAWN_COLUMN: number = 3;
        static MAX_LIVES: number = 3;
        static ENEMIES_SPEED: number = 1400;
        static PLAYER_BULLET_SPEED: number = 340;
        static PLAYER_BULLET_FREQUENCY: number = 260;
        static ENEMY_BULLET_SPEED: number = 260;
        static ENEMY_BULLET_FREQUENCY: number = 1400;
        ...
        createEnemies() {
            for (var x = 0; x < Level4.SPAWN_ROW; x++) {
                for (var y = 0; y < Level4.SPAWN_COLUMN; y++) {
                    var enemy: Phaser.Sprite = this.enemies.create((x * 36) + 70, (y * 44) + 30, 'enemies');
            ...
            var tween: Phaser.Tween = this.add.tween(this.enemies).to({ x: 200 }, Level4.ENEMIES_SPEED, Phaser.Easing.Linear.None, true, 0, true, true);
            tween.onComplete.add(this.moveEnemiesDown, this);
        }

        moveEnemiesDown() {
            var tween: Phaser.Tween = this.add.tween(this.enemies).to({ y: this.enemies.y + 20 }, Level4.ENEMIES_SPEED / 4, Phaser.Easing.Linear.None, true, 0);
